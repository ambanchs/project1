...
joined <- joined %>% mutate(Country.size= ifelse(`Population in thousands (2017)`>mean(`Population in thousands (2017)`), "large", "small"))
joined %>% select(1,15,66)
...
```{R}
joined %>% ggplot(aes(Happiness.Score, `GDP per capita (current US$)`)) + geom_point(aes(color=Region)) + geom_smooth()
```
